# -*- coding: utf-8 -*-
"""
Created on Mon Mar  5 14:53:08 2018

@author: mbcxajg2
****************************************
    DATA ANALYSIS: DROP 2 TOP VIEW
****************************************
"""
...
r1_data = np.loadtxt('Top_view_drop_2_data_run1.txt')    
r2_data = np.loadtxt('Top_view_drop_2_data_run2.txt')    
data_len = len(r1_data)

#==========================================
#                  TIME
#==========================================
time = r1_data[:, 0]        # time values are same for r1_data to r2_data

#===========================================
#     MEAN RADIUS (UM) AND ERROR
#===========================================
# Calculate mean radius values
mean_r = (r1_data[:, 1] + r2_data[:, 1]) / 2
...
for i in range(data_len):
    mean_r_err[i] = np.std([r1_data[i][1], r2_data[i][1]]) / math.sqrt(1)
...
q_u_expected_err = np.sqrt((1/(458-3))*(np.sum(np.power(u-q_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 2 Top View [Quadratic Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
# Calculate statistically expected errors for the fit
c_u_expected_err = np.sqrt((1/(458-4))*(np.sum(np.power(u-c_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 2 Top View [Cubic Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
# Calculate statistically expected errors for the fit
l_u_expected_err = np.sqrt((1/(458-2))*(np.sum(np.power(u-l_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 2 Top View [Linear Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
# Calculate statistically expected errors for the fit
g_u_expected_err = np.sqrt((1/(458-3))*(np.sum(np.power(u-g_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 2 Top View [de Gennes Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
print('[de Gennes Fit] Fitted Equation: ax^2 + b')
...
# Calculate statistically expected errors for the fit
v_u_expected_err = np.sqrt((1/(458-4))*(np.sum(np.power(u-v_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 2 Top View [Cox Voinov Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
