# -*- coding: utf-8 -*-
"""
Created on Sun Mar  4 20:44:12 2018

@author: mbcxajg2
****************************************
    DATA ANALYSIS: DROP 1 SIDE VIEW
****************************************
"""
...
r1_data = np.loadtxt('Side_view_data_run1.txt')    
r2_data = np.loadtxt('Side_view_data_run2.txt')    
r3_data = np.loadtxt('Side_view_data_run3.txt')    
r4_data = np.loadtxt('Side_view_data_run4.txt')    
r5_data = np.loadtxt('Side_view_data_run5.txt')    
r6_data = np.loadtxt('Side_view_data_run6.txt')
data_len = len(r1_data)

#==========================================
#                  TIME
#==========================================
time = r1_data[:, 0]        # time values are same for r1_data to r6_data
   
#===========================================
#     MEAN RADIUS (um) AND ERROR
#===========================================
# Calculate mean radius values
mean_r = (r1_data[:, 1] + r2_data[:, 1] + r3_data[:, 1] + r4_data[:, 1] + r5_data[:, 1] + r6_data[:, 1]) / 6
...
for i in range(data_len):
    mean_r_err[i] = np.std([r1_data[i][1], r2_data[i][1], r3_data[i][1], r4_data[i][1], r5_data[i][1], r6_data[i][1]]) / math.sqrt(5)
...
#=================================================
# Plot
plt.errorbar(theta, u, xerr=theta_err, yerr=u_err, marker='o', linestyle='None')
...
q_u_expected_err = np.sqrt((1/(42-3))*(np.sum(np.power(u-q_u, 2)))) # this value is a constant
q_u_expected_err = q_u_expected_err * np.ones(len(q_u)) # create an array of this value

# Plot fitted u against theta
plt.errorbar(theta, q_u, yerr=u_err, marker='o')
plt.ylabel('Contact Line Speed / um/s')
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 1 Side View [Quadratic Fit]\n Contact Line Speed vs Contact Angle')
plt.show()

# Plot residuals with statistically expected errors
plt.errorbar(theta, q_u-u, yerr=q_u_expected_err, marker='o', linestyle='None')
plt.ylim(16,-16)
...
plt.errorbar(theta, q_u-u, yerr=u_err, marker='o', linestyle='None')
plt.ylim(16,-16)
plt.ylabel('Residuals / um/s')
plt.xlabel('Contact Angle / degree')
plt.grid()
...
# Calculate statistically expected errors for the fit
c_u_expected_err = np.sqrt((1/(42-3))*(np.sum(np.power(u-c_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 1 Side View [Cubic Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
c_reduced_chisq = c_chisq/ (42-3)
print('[Cubic Fit] Chi-Sq:', c_chisq)
print('[Cubic Fit] Reduced Chi-Sq:', c_reduced_chisq)

# Comment
print('[Cubic Fit] Comment: Reduce Chi-Sq values not as good as quadratic fit.')
...
# Calculate statistically expected errors for the fit
l_u_expected_err = np.sqrt((1/(42-2))*(np.sum(np.power(u-l_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 1 Side View [Linear Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
# Calculate statistically expected errors for the fit
g_u_expected_err = np.sqrt((1/(42-3))*(np.sum(np.power(u-g_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 1 Side View [de Gennes Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
...
# Calculate statistically expected errors for the fit
v_u_expected_err = np.sqrt((1/(42-4))*(np.sum(np.power(u-v_u, 2)))) # this value is a constant
...
plt.xlabel('Contact Angle / degree')
plt.grid()
plt.title('Drop 1 Side View [Cox Voinov Fit]\n Contact Line Speed vs Contact Angle')
plt.show()
